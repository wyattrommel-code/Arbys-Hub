...
Response evidence is parsed, allowlisted, and regenerated as namespace-normalized XML. Customer/payment records, freeform notes and unknown fields are excluded. It is clearly labeled **sanitized XML, not an unmodified raw response**. Valid responses up to 64 KiB after sanitizing retain XML; larger ones retain a size-omission marker, safe error explanation if available, byte count and SHA-256 hash. Non-XML bodies are not archived. Detailed response XML expires after 24 hours; metadata/request history after 30 days. Export the relevant evidence before that window. Logs begin with this release; earlier requests are represented only by the saved setup evidence, not reconstructed logs.

### PAR error explanations (integration version 2026-09-25.1)

Failed GetOrders results retain only the top-level scalar Message. SOAP 1.1 faults retain faultcode/faultstring, including on non-2xx responses; fault detail and faultactor are discarded. The redacted explanation is appended to the existing call error and last-error fields, so call details and daily/single-call JSON exports retain it for the existing 30-day metadata window. No new database column or migration is required. A failed request still cannot replace the saved sales snapshot, and this change does not add PAR requests or retries.

Diagnostic free text is untrusted: exact configured tokens (including URL/base64 forms), credential/customer/payment field assignments, quoted values, embedded XML/JSON payloads, URLs, email addresses, UUIDs, long digit sequences and token-like strings are removed before the message is capped at 1,000 characters plus a truncation marker. Nested Message objects and messages inside order records are not captured. Heuristic redaction is intentionally conservative and cannot identify every possible unlabeled personal detail; diagnostic exports remain GM-only and must be reviewed before external sharing. React renders errors as text, never HTML. Error-body reads are capped at 256 KiB; successful daily reads retain the existing 20 MiB bound. Previously discarded messages cannot be reconstructed from a response hash.
...
Individual diagnostic messages larger than 16 KiB are replaced entirely with an omission marker before redaction processing, rather than scanning or retaining an unbounded payload.
