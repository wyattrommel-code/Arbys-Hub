...
export default function BrinkCallLog({ date }) {
  ...
  return <div className="mt-6 border-t border-gray-200 pt-4">
    ...
    <p className="mt-1 text-xs text-gray-500">All Hub-to-PAR calls from logging activation onward. History and redacted PAR error messages: 30 days. Sanitized response XML: 24 hours, up to 64 KiB per call. Customer/payment records are excluded; diagnostic text is filtered for credentials and sensitive values. Messages discarded by older versions cannot be recovered.</p>
    ...
    {detail && <div className="mt-2 rounded-lg border border-gray-200 p-3 text-xs">
      <p>Integration version: {detail.integration_version || "Before diagnostic logging"}</p>
      <p className="mt-2 font-bold">Database recovery</p>
      {detail.diagnostics?.length ? <><p className="mt-1">{detail.status === "success" ? "This call completed after recovering from the database errors below." : "Database errors encountered during this run:"}</p><ul className="mt-2 space-y-1">{detail.diagnostics.map((d, index) => <li key={index}>{d.stage} · attempt {d.attempt} · {d.code || d.kind} · {d.status ? `HTTP ${d.status}` : "No HTTP response"}</li>)}</ul></> : <p className="mt-1">{detail.integration_version ? "No database retries recorded." : "Detailed database diagnostics were not recorded for this older call."}</p>}
    </div>}
  </div>;
}
