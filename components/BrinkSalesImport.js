...
export default function BrinkSalesImport() {
  ...
  useEffect(() => {
    ...
    const refresh = () => fetch(`/api/integrations/brink?date=${date}`, { signal: controller.signal }).then(async (r) => {
  ...
  async function toggleAutomatic() {
    setBusy(true); setError("");
    try {
      const enabled = !data?.state?.automatic_enabled;
      const response = await fetch("/api/integrations/brink/automation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enabled }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error);
      setData((old) => ({ ...old, state: { ...old.state, automatic_enabled: result.enabled } }));
  ...
  return <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
    ...
    <div className="mt-4 flex flex-wrap items-end gap-3">
      ...
      <button type="button" disabled={busy || !data?.configured} onClick={toggleAutomatic} className="rounded-md bg-[#C8102E] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Working…" : data?.state?.automatic_enabled ? "Pause automatic sync" : "Enable automatic sync"}</button>
    </div>
    ...
    <p className="mt-3 text-sm text-gray-600">{data?.state?.automatic_enabled ? "Automatic sync is ON · checks PAR every 3 minutes, even with this page closed. This view refreshes every 30 seconds." : "Automatic sync is paused."}</p>
    <p className="mt-1 text-xs text-gray-500">Orders appear after the register sends them to PAR and the next sync finishes. Previous-day reconciliation runs each morning. Repeated syncs do not duplicate sales.</p>
    {data?.state?.automatic_enabled && data.state.last_success_at && Date.now() - Date.parse(data.state.last_success_at) > 7 * 60000 && <p role="status" className="mt-2 text-sm text-amber-800">Sales have not synced in over 7 minutes. Check the call log below.</p>}
