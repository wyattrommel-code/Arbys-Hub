...
export { computeWorkedMinutes, toStoredMinutes } from "./timecards";
...
export async function fetchClockRoster(supabase) {
  ...
  for (const punch of openPunches || []) {
    if (punch.employee_id && !openById.has(punch.employee_id)) {
      openById.set(punch.employee_id, punch.clock_in);
    }
  }
  ...
  return employees
    .map((emp) => ({
      ...
      clocked_in: openById.has(emp.id),
      clock_in: openById.get(emp.id) || null,
    }))
