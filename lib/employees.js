 ...
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 */
export async function fetchEmployeeByPin(supabase, pin, storeId = STORE_ID, select = "id, first_name, last_name, role") {
  if (!/^\d{4}$/.test(String(pin || "").trim())) return null;
  const { data: employeeId, error: pinError } = await supabase.rpc("hub_employee_by_pin", { p_pin: String(pin).trim(), p_store_id: storeId });
