...
import { getEffectiveAccess } from "./roles";
import {
  SCHEDULE_STORE_ID,
...
const SHIFT_SELECT =
  "id, shift_date, employee_id, employee_name, jolt_employee_id, scheduled_start, scheduled_end, unpaid_break_minutes, scheduled_hours, role, station, week_start_date";
...
export function shiftAssignedTo(shift, employee) {
  if (!shift || !employee || isUnassignedShift(shift)) return false;
  return shiftBelongsToEmployee(shift, employee);
}

export { findEmployeeForShift };
...
export function serializeShiftCard(shift, extras = {}) {
  ...
  const unassigned = isUnassignedShift(shift);
  return {
    id: shift.id,
    ...
    employee_name: unassigned ? "Unassigned" : shift.employee_name || "",
    unassigned,
    ...extras,
  };
}
...
function assigneePatch(employee) {
  return assigneeFields(employee);
}
...
export async function loadMarketplace(supabase, sessionEmployee) {
  ...
  const access = await getEffectiveAccess(supabase, sessionEmployee.employee_id);
  const me = {
    ...sessionEmployee,
  ...
  const others = allShifts.filter((s) => !shiftAssignedTo(s, me) && !isUnassignedShift(s));
  const unassigned = allShifts.filter((s) => isUnassignedShift(s));
...
export async function loadApprovalQueue(supabase, sessionEmployee) {
  const hub = await fetchHubEmployee(supabase, sessionEmployee.employee_id);
  const access = await getEffectiveAccess(supabase, sessionEmployee.employee_id);
  const me = { ...sessionEmployee, ...hub, role: hub?.role || sessionEmployee.role, access_tier: access };
  const today = getStoreToday();
...
export async function reviewOffer(supabase, sessionEmployee, offerId, action, note) {
  ...
  if (
    ...
    offer.offered_by_id &&
    shift.employee_id &&
    String(shift.employee_id) !== String(offer.offered_by_id)
  ) {
