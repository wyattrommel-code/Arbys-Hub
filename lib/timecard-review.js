...
export function matchesPunchReview(punch, filter) {
  if (filter === "pending") return Boolean(punch.pending_approval);
  ...
  if (filter === "long") return Boolean(punch.clock_out && Date.parse(punch.clock_out) - Date.parse(punch.clock_in) > 16 * 3600000);
