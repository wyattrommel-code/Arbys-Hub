...
export function isUnassignedShift(shift) {
  if (!shift) return true;
  if (shift.employee_id) return false;
  return isUnassignedName(shift.employee_name);
}

function isNamePlaceholderId(id) {
  return String(id || "").startsWith("name:");
}

/** Hub employee uuid, or null for Unassigned / name-only placeholder rows. */
export function hubEmployeeUuid(employee) {
  if (!employee || employee.isUnassigned) return null;
  const id = employee.id || employee.employee_id || null;
  if (!id || isNamePlaceholderId(id)) return null;
  return String(id);
}

export function assigneeFields(employee) {
  if (!employee || employee.isUnassigned) {
    return {
      employee_id: null,
      employee_name: UNASSIGNED_EMPLOYEE_NAME,
      jolt_employee_id: null,
    };
  }
  const name =
    employee.fullName || `${employee.first_name || ""} ${employee.last_name || ""}`.trim() || UNASSIGNED_EMPLOYEE_NAME;
  return {
    employee_id: hubEmployeeUuid(employee),
    employee_name: name,
    jolt_employee_id: employee.jolt_employee_id || null,
  };
}

/** Exact full-name resolve only — never last-name fuzzy (Campbell collision). */
export function resolveEmployeeId(employees, { employee_id, employee_name } = {}) {
  if (employee_id) return String(employee_id);
  if (isUnassignedName(employee_name)) return null;
  const target = nameKey(employee_name);
  if (!target) return null;
  const match = (employees || []).find((emp) => {
    if (emp.isUnassigned || isNamePlaceholderId(emp.id)) return false;
    const full = nameKey(emp.fullName || `${emp.first_name || ""} ${emp.last_name || ""}`.trim());
...
export function shiftBelongsToEmployee(shift, employee) {
  ...
  const empId = hubEmployeeUuid(employee);
  const shiftId = shift.employee_id ? String(shift.employee_id) : "";
  if (empId && shiftId) return empId === shiftId;
  if (empId) return false;
  if (employee.isUnassigned) return isUnassignedShift(shift);
  return false;
}
...
export function findEmployeeForShift(employees, shift) {
  ...
  if (shift.employee_id) {
    return list.find((emp) => !emp.isUnassigned && String(emp.id) === String(shift.employee_id)) || null;
  }
  if (isUnassignedName(shift.employee_name)) {
    return list.find((emp) => emp.isUnassigned) || null;
  }
  return (
    list.find(
      (emp) => emp.isSynthetic && isNamePlaceholderId(emp.id) && nameKey(emp.fullName) === nameKey(shift.employee_name)
    ) || null
  );
}
...
export function employeeSortRank(emp) {
  ...
  const role = String(emp?.access_tier || emp?.role || "").toLowerCase();
