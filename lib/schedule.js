...
export const SHIFT_PALETTE = [
  { label: "Open", start: "05:00:00", end: "10:00:00", role: "Open" },
  { label: "Breakfast", start: "05:00:00", end: "11:00:00", role: "Breakfast" },
  { label: "Morning", start: "07:00:00", end: "15:00:00", role: "Morning" },
  { label: "Mid", start: "10:00:00", end: "16:00:00", role: "Mid Shift" },
  { label: "Day Lead", start: "07:00:00", end: "16:00:00", role: "Day Lead" },
  { label: "Night", start: "15:00:00", end: "22:00:00", role: "Night" },
  { label: "Night Lead", start: "15:00:00", end: "23:00:00", role: "Night Lead" },
  { label: "Closing", start: "16:00:00", end: "00:00:00", role: "Closing" },
];

const DEFAULT_SHIFT = { start: "09:00:00", end: "17:00:00", role: "" };
...
export function computeScheduledHours(start, end) {
...
export function defaultShiftForRole(primaryRole) {
  const role = String(primaryRole || "").trim();
  const match = SHIFT_PALETTE.find(
    (item) => item.role.toLowerCase() === role.toLowerCase()
  );
  if (match) return { ...match };
  return { ...DEFAULT_SHIFT, role };
}
...
export function employeeSortRank(emp) {
  ...
  if (role === "shift_lead" || emp?.is_shift_lead) return 1;
  if (emp?.isSynthetic) return 3;
  return 2;
}
