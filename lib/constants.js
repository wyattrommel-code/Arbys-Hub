export const STORE_ID = "07462";

export const SESSION_COOKIE = "hub_session";
export const SESSION_MAX_AGE_SEC = 8 * 60 * 60;

export const STORE_TIMEZONE = "America/Denver";

export const CHECKLIST_PHOTOS_BUCKET = "checklist-photos";

/**
 * Punch photos for the time clock. Public read is fine for now; tighten later
 * alongside the known PIN-auth security item.
 */
export const PUNCH_PHOTOS_BUCKET = "punch-photos";

/**
 * Account avatars for the time-clock roster. Public read is fine for now; tighten later
 * alongside the known PIN-auth security item.
 */
export const PROFILE_PHOTOS_BUCKET = "profile-photos";

export const MANAGER_ROLES = new Set(["shift_lead", "assistant_manager", "gm"]);

/** Hub roles allowed to submit deployments or sign as closing manager. */
export const SHIFT_LEAD_PLUS_ROLES = ["shift_lead", "assistant_manager", "gm"];

export const DAY_OF_WEEK_OPTIONS = [
  { value: "", label: "Every day" },
  { value: "0", label: "Sun" },
  { value: "1", label: "Mon" },
  { value: "2", label: "Tue" },
  { value: "3", label: "Wed" },
  { value: "4", label: "Thu" },
  { value: "5", label: "Fri" },
  { value: "6", label: "Sat" },
];

export const SHIFT_OPTIONS = ["AM", "PM", "BOTH"];

export const VERIFICATION_METHOD_OPTIONS = [
  { value: "checkbox", label: "Checkbox" },
  { value: "photo", label: "Photo" },
  { value: "photo_ai", label: "Photo+AI" },
  { value: "signature", label: "Signature" },
];

/** Display order for checklist role sections. */
export const CHECKLIST_ROLE_ORDER = [
  "opener",
  "morning",
  "line_closing",
  "fryer_closing",
  "running_station_closing",
  "dt_closing",
  "lobby_closing",
  "manager_closing",
  "any",
];

export const CHECKLIST_ROLE_LABELS = {
  opener: "Opening Checklist",
  morning: "Morning Checklist",
  line_closing: "Line Closing",
  fryer_closing: "Fryer Closing",
  dt_closing: "Drive Thru Closing",
  lobby_closing: "Lobby Closing",
  running_station_closing: "Front Counter",
  manager_closing: "Manager Closing",
  any: "General",
};

/** Roles shown on home widget during AM shift (before 2pm). */
export const CHECKLIST_ROLES_AM = new Set(["opener", "morning"]);

/** Roles shown on home widget during PM shift. */
export const CHECKLIST_ROLES_PM = new Set([
  "line_closing",
  "fryer_closing",
  "running_station_closing",
  "dt_closing",
  "lobby_closing",
  "manager_closing",
]);

export const CHECKLIST_ROLE_OPTIONS = CHECKLIST_ROLE_ORDER.map((value) => ({
  value,
  label: CHECKLIST_ROLE_LABELS[value],
}));
