...
export function authorizeDataRequest({ actor, table, method, search, body, prefer = "" }) {
  ...
  for (const key of ["select", "limit", "offset", "order", "on_conflict", "columns"]) {
    if (params.getAll(key).length > 1) throw forbidden("Duplicate control parameter");
  }
  // supabase-js adds a quoted column list to bulk inserts/upserts. Validate it,
  // then let PostgREST infer columns from the fully validated server-owned body.
  if (params.has("columns")) {
    const columns = params.get("columns").split(",").map((c) => c.replace(/^"([a-z_]+)"$/, "$1"));
    if (method !== "POST" || columns.some((c) => !schema[table].includes(c))) throw forbidden("Write column access denied");
