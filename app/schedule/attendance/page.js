...
export default function ScheduleAttendancePage() {
  ...
  const comparisonsByDay = useMemo(() => {
    ...
    for (const shift of scheduledRowsForOt) {
      ...
      const start = weekStartFromDateStr(shift.shift_date);
      const runKey = `${employeeKey}::${start}`;
  ...
  const employeeCards = useMemo(() => {
    ...
    const cards = [...byEmployee.values()].map((group) => {
      ...
      const avgLate =
        lateIn > 0
          ? sorted.flatMap((s) => s.flags.filter((f) => f.type === "LATE IN").map((f) => f.minutes)).reduce((a, b) => a + Math.abs(b), 0) / lateIn
          : 0;
      const avgEarly =
        earlyOut > 0
          ? sorted.flatMap((s) => s.flags.filter((f) => f.type === "EARLY OUT").map((f) => f.minutes)).reduce((a, b) => a + Math.abs(b), 0) / earlyOut
          : 0;
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 px-4 py-5">
      ...
      {tab === "WEEK VIEW" ? (
        <>
          ...
          {loading ? (
            <div className="rounded-xl border border-zinc-200 bg-white p-4 text-sm text-zinc-500 shadow-sm">Loading week...</div>
          ) : !hasWeekSchedule ? (
            <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
              <p className="font-semibold">No schedule for this week</p>
              <p className="mt-1 text-xs text-zinc-500">
                Build it in <Link href="/schedule/builder" className="text-[#C8102E] underline">Schedule Builder</Link>.
              </p>
            </div>
          ) : (
            <>
              {!hasWeekLabor ? (
                <div className="rounded-xl border border-zinc-200 bg-white p-3 text-xs text-zinc-600 shadow-sm">
                  Labor data not yet available for this period.
                </div>
              ) : null}
              <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white p-2 shadow-sm">
                <div className="grid min-w-[840px] grid-cols-7 gap-2">
                  {weekDays.map((d) => (
                    <div key={d} className="min-w-[120px] rounded-lg border border-zinc-200 p-2">
                      ...
                      <div className="space-y-2">
                        {(weekComparisonsByDay.get(d) || []).map((cmp, idx) => (
                          <div key={`${d}-${idx}`} className="rounded-md border border-zinc-200 p-2 text-xs">
                            <div className="flex items-center justify-between">
                            ...
                            <p className="mt-1 text-zinc-600">{fmtTime(cmp.shift.scheduled_start)} → {fmtTime(cmp.shift.scheduled_end)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                            ...
                            <tr key={`${day}-${idx}`} className={`border-t border-zinc-100 dark:border-zinc-800 ${r.status.label === "NO SHOW" ? "bg-red-50/70" : ""}`}>
                              ...
                              <td className="px-2 py-1">
                                {r.shift && r.otWarningHours > 0 ? (
                                  <span className="rounded bg-amber-100 px-1 py-0.5 text-[10px] font-semibold text-amber-800">OT: {r.otWarningHours.toFixed(1)} hrs</span>
                                ) : "—"}
                              </td>
          ...
          {employeeCards.map((emp) => {
            ...
            return (
              <article key={emp.key} className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div className="mt-3 text-xs text-zinc-700">On Time: {emp.onTime} | Late In: {emp.lateIn} | Early Out: {emp.earlyOut} | No Show: {emp.noShow}</div>
                {open ? (
                  <div className="mt-3 overflow-x-auto border-t border-zinc-200 pt-3">
                    <table className="min-w-[760px] text-left text-xs">
                      <thead className="bg-zinc-50"><tr><th className="px-2 py-1">Date</th><th className="px-2 py-1">Role</th><th className="px-2 py-1">Sched</th><th className="px-2 py-1">Actual</th><th className="px-2 py-1">Status</th><th className="px-2 py-1">Flags</th></tr></thead>
                      <tbody>
