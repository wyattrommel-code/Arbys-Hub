...
function isSingleRowItem(item) {
  return !isMultiRowSandwichItem(item);
}

function buildInitialSandwichRows(items) {
  const out = {};
  for (const item of items) {
    if (isMultiRowSandwichItem(item)) {
      out[sandwichRowKey(item)] = [createSandwichRow()];
    }
  }
  return out;
}
...
export default function WastePage() {
  ...
  /** sandwich waste item id -> sandwich rows */
  const [sandwichRowsByItem, setSandwichRowsByItem] = useState({});
  ...
  const sandwichItem = useMemo(
    () => items.find((i) => isMultiRowSandwichItem(i)),
    [items]
  );

  const setSandwichRows = useCallback((itemId, rows) => {
    setSandwichRowsByItem((prev) => ({ ...prev, [itemId]: rows }));
  }, []);
  ...
  const totals = useMemo(() => {
    ...
    for (const item of items) {
      if (!isSingleRowItem(item)) continue;
      ...
      retail += q * (Number(item.unit_retail_value) || 0);
    }

    if (sandwichItem) {
      const rows = sandwichRowsByItem[sandwichRowKey(sandwichItem)] || [];
      const w = Number(sandwichItem.unit_wholesale_cost) || 0;
      for (const row of rows) {
        const q = parsePositiveQty(row.qty);
        if (q <= 0) continue;
        const opt = row.optionIdx >= 0 ? SANDWICH_OPTIONS[row.optionIdx] : null;
  ...
  }, [items, quantities, sandwichRowsByItem, sandwichItem]);
  ...
  const resetForm = useCallback(() => {
    ...
    setSubmittedBy("");
    setQuantities({});
    setSandwichRowsByItem(buildInitialSandwichRows(items));
    setSubmitError("");
    setConfirmation(null);
  }, [items]);

  function appendVariantLogRows(rows, item, q, variantName, variantRetail) {
    const base = {
      item_id: item.id,
      item_name: itemDisplayName(item),
      quantity: q,
      shift,
      submitted_by: submittedBy.trim(),
      log_date: logDate,
      total_wholesale_cost: q * (Number(item.unit_wholesale_cost) || 0),
      total_retail_loss: q * variantRetail,
      sandwich_type: variantName,
      sandwich_retail_price: variantRetail,
    };
    rows.push(base);
    if (effectiveCategory(item) !== "Bread") {
      return base.total_retail_loss;
    }
    return 0;
  }
  ...
  async function handleSubmit(e) {
    ...
    for (const item of items) {
      ...
      rows.push(base);
      if (effectiveCategory(item) !== "Bread") {
        totalRetailForConfirmation += base.total_retail_loss;
      }
    }

    if (sandwichItem) {
      const variantRows = sandwichRowsByItem[sandwichRowKey(sandwichItem)] || [];
      for (const row of variantRows) {
        const q = parsePositiveQty(row.qty);
        if (q <= 0) continue;
        const opt = row.optionIdx >= 0 ? SANDWICH_OPTIONS[row.optionIdx] : null;
        if (!opt) {
          setSubmitError("Select a sandwich type for each row with a quantity.");
          return;
        }
        totalRetailForConfirmation += appendVariantLogRows(rows, sandwichItem, q, opt.name, opt.price);
      }
    }
  ...
  function renderSingleRowItem(item) {
    const id = item.id;
    const name = itemDisplayName(item);
    const unit = item.unit_label ?? "";
    const label = unit && unit.trim() ? `${name} — ${unit}` : name;
    const qRaw = quantities[id] ?? "";
    const q = parsePositiveQty(qRaw);
    const cat = effectiveCategory(item);
    const isBread = cat === "Bread";
    const lineRetail = isBread ? 0 : q * (Number(item.unit_retail_value) || 0);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="shrink-0 text-right">
            <p className="flex flex-wrap items-baseline justify-end gap-1.5 text-xs font-medium text-zinc-500 dark:text-zinc-400">
              ...
              >
                {formatMoney(isBread ? lineWholesale : lineRetail)}
              </span>
              <span className="normal-case tracking-normal">{isBread ? "wholesale" : "retail lost"}</span>
            </p>
          </div>
        </div>
      </li>
    ...
    return (
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col gap-6 px-4 py-8">
        <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">Submission recorded</p>
          <dl className="mt-4 space-y-3 text-sm text-zinc-700 dark:text-zinc-300">
            <div className="flex justify-between gap-4">
              ...
              <dd className="font-medium text-zinc-900 dark:text-zinc-100">{confirmation.logDate}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-zinc-500 dark:text-zinc-400">Shift</dt>
              <dd className="font-medium text-zinc-900 dark:text-zinc-100">{confirmation.shift}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-zinc-500 dark:text-zinc-400">Submitted by</dt>
              <dd className="font-medium text-zinc-900 dark:text-zinc-100">{confirmation.submittedBy}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-zinc-500 dark:text-zinc-400">Items logged</dt>
              <dd className="font-medium text-zinc-900 dark:text-zinc-100">{confirmation.itemCount}</dd>
            </div>
            <div className="flex justify-between gap-4 border-t border-zinc-100 pt-3 dark:border-zinc-800">
              <dt className="text-zinc-500 dark:text-zinc-400">Total Retail Loss</dt>
      ...
      <div className="flex-1 space-y-6 px-4 pb-44 pt-6">
        {loadState.status === "loading" && (
          <div className="rounded-xl border border-zinc-200 bg-white p-5 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Loading waste items…</p>
        ...
        {loadState.status === "error" && (
          <div className="space-y-3 rounded-xl border border-red-300 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/50">
            <p className="text-sm font-semibold text-red-900 dark:text-red-100">Could not load waste items</p>
        ...
        {loadState.status === "empty" && (
          <div className="space-y-3 rounded-xl border border-amber-300 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/40">
            <p className="text-sm font-semibold text-amber-950 dark:text-amber-100">No waste items to show</p>
            <p className="whitespace-pre-wrap text-sm text-amber-900 dark:text-amber-200">{loadState.message}</p>
            <button
              type="button"
              onClick={retryFetch}
          ...
          <>
            <div className="space-y-4 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div>
                ...
                <div className="mt-1 grid grid-cols-3 gap-2" role="group" aria-labelledby="shift-label">
                  {["AM", "Mid", "Night"].map((s) => (
                    <button
                      key={s}
                      type="button"
            ...
            {grouped.map(([category, catItems]) => (
              <section key={category} className="space-y-3">
                <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-50">{category}</h2>
                <ul className="space-y-3">
                  {catItems.filter(isSingleRowItem).map(renderSingleRowItem)}

                  {category === "Other" && sandwichItem ? (
                    <MultiRowVariantCard
                      title={itemDisplayName(sandwichItem)}
                      subtitle={sandwichItem.unit_label ? `Unit: ${sandwichItem.unit_label}` : "each"}
                      rows={sandwichRowsByItem[sandwichRowKey(sandwichItem)] || [createSandwichRow()]}
                      onRowsChange={(rows) => setSandwichRows(sandwichRowKey(sandwichItem), rows)}
                      options={SANDWICH_OPTIONS}
                      optionLabel="Select sandwich…"
                      formatOption={(opt) => `${opt.name} — ${formatMoney(opt.price)}`}
                      rowRetail={(row) => {
                        const q = parsePositiveQty(row.qty);
                        const opt = row.optionIdx >= 0 ? SANDWICH_OPTIONS[row.optionIdx] : null;
                        return opt ? q * opt.price : 0;
                      }}
                      formatMoney={formatMoney}
                      addButtonLabel="Add another sandwich"
                    />
                  ) : null}
                </ul>
              </section>
      ...
      <footer className="sticky bottom-0 z-10 border-t border-zinc-200 bg-white/95 px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[0_-4px_12px_rgba(0,0,0,0.06)] backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/95 dark:shadow-[0_-4px_12px_rgba(0,0,0,0.3)]">
        <div className="mx-auto w-full max-w-md space-y-1">
          ...
          <div className="flex items-baseline justify-between gap-4">
            <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Total Retail Loss</span>
            <span className="text-2xl font-bold tabular-nums text-red-600 dark:text-red-400">
              {formatMoney(totals.retail)}
            </span>
          </div>
          <div className="flex items-baseline justify-between gap-4">
            <span className="text-xs text-zinc-500 dark:text-zinc-500">Total Wholesale Cost</span>
            <span className="text-sm tabular-nums text-zinc-500 dark:text-zinc-400">{formatMoney(totals.wholesale)}</span>
          </div>
          <div className="flex items-baseline justify-between gap-4">
            <span className="text-xs font-medium text-orange-700 dark:text-orange-400/90">Bread Waste Cost</span>
