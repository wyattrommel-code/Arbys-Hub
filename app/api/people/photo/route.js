...
export async function POST(request) {
  ...
  try {
    ...
    if (!employeeId) {
      return secureJson({ ok: false, error: "Employee is required." }, { status: 400 });
    }
    if (!file || typeof file === "string" || typeof file.arrayBuffer !== "function") {
      return secureJson({ ok: false, error: "Photo file is required." }, { status: 400 });
    }
    if (file.type && !ALLOWED_TYPES.has(String(file.type).toLowerCase())) {
      return secureJson({ ok: false, error: "Use a JPEG, PNG, or WebP photo." }, { status: 400 });
    }
    const supabase = getSupabaseServer();
    if (!(await employeeExists(supabase, employeeId))) {
      return secureJson({ ok: false, error: "Employee not found." }, { status: 404 });
    }
    const url = await uploadProfilePhoto(supabase, employeeId, file);
    return secureJson({ ok: true, profile_photo_url: url });
  } catch (err) {
    return secureJson(
...
export async function DELETE(request) {
  ...
  try {
    ...
    return secureJson({ ok: true, profile_photo_url: null });
  } catch (err) {
    return secureJson(
